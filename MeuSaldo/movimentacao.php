...
//Lucro Venda
$sqlLucroVendas = "SELECT SUM(lucroVenda) AS lucroVendas
                   FROM Vendas V
                   INNER JOIN Anuncios A
                   ON V.Anuncios_idAnuncios = A.idAnuncio
                   WHERE A.Usuarios_idUsuario = $idUsuario";

$resultadoLucroVendas = mysqli_query($conn, $sqlLucroVendas);

$dadosLucroVendas = mysqli_fetch_assoc($resultadoLucroVendas);

$lucroVendas = $dadosLucroVendas['lucroVendas'] ?? 0;
...
$lucroVenda = $lucroVenda + $_SESSION['lucroVenda'];

//LUCRO TOTAL
$lucroTotal = ($outrasReceitas - $despesaTotalizada) + $lucroVendas;
